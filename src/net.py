...
class Net(object):
    ...
    def conv_layer(self, t, nmaps, name, act=tf.nn.relu, drop=False, norm=True, pad='SAME', shape=None, stride=STRIDE):
        ...
        with tf.variable_scope(name):
            ...
            # Add bias
            nmaps = shape[3]
            bias = tf.get_variable('bias', initializer=tf.constant(.1, shape=[nmaps]))
            maps = tf.nn.bias_add(maps_, bias)

            # Training related ops
            maps = self.instance_normalize(maps) if norm else maps
    ...
    def instance_normalize(inputs):
        ...
        with tf.variable_scope('instance_normalization'):
            ...
            shift = tf.get_variable('shift', initializer=tf.constant(.1, shape=[channels]))
